...
It is also possible that a single chromophore is doubly excited \bra{m_{2}}, but we have omitted this in the diagram for brevity.
...
\centering
\begin{tikzpicture}[x=.5cm,y=.5cm]
\node[above] at (2, 4) {$R''_\mathrm{SE}(t_{1},t_{2},t_{3})$};
%stimulated emission
\draw (0,0) -- (0,4);
\draw (4,0) -- (4,4);
\draw[\arw] (-2,-1) -- (0 ,0);
...
\centering
\begin{tikzpicture}[x=.5cm,y=.5cm]
\node[above] at (2, 4) {$R''_\mathrm{ESA}(t_{1},t_{2},t_{3})$};
%excited state absorption
\draw (0,0) -- (0,4);
\draw (4,0) -- (4,4);
\draw[\arw] (-2,-1)  -- (0 ,0);
...
\centering
\begin{tikzpicture}[x=.5cm,y=.5cm]
\node[above] at (2, 4) {$R''_\mathrm{2QI}(t_{1},t_{2},t_{3})$};
%two-quantum
\draw (0,0) -- (0,4);
\draw (4,0) -- (4,4);
\draw[\arw] (-2,-1)  -- ( 0 ,0);
...
\centering
\begin{tikzpicture}[x=.5cm,y=.5cm]
\node[above] at (2, 4) {$R''_\mathrm{2QII}(t_{1},t_{2},t_{3})$};
%two-quantum
\draw (0,0) -- (0,4);
\draw (4,0) -- (4,4);
\draw[\arw] (-2,-1)  -- (0 ,0);
...
Following Jansen and Knoester,\cite{Jansen2006} the exponential integral operators, or propagators, can be estimated numerically by breaking them into a series of short intervals, during which the Hamiltonian is assumed constant
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
The complex conjugate is given by the usual rules of matrix multiplication
\begin{equation}
\begin{split}
...
\clearpage
\section{Isotropically Averaged Spectrum}
If we assume that our system corresponds to one configuration of an isotropic average, we can calculate the 2DIR spectrum for the isotropically averaged system by taking averages over the different combinations of laser pulse polarizations.
Following Hochstrasser\cite{Hochstrasser2001} and Jansen\cite{Jansen2006} we can write
\begin{equation}
\begin{split}
\left\langle I(\omega_{1},\omega_{3},t_{2})_{ZZZZ}\right\rangle=
\sum_{i,j={X,Y,Z}}
&\frac{1}{5}I(\omega_{1},\omega_{3},t_{2})_{iiii}+
\frac{1}{15}I(\omega_{1},\omega_{3},t_{2})_{iijj}+\\
&\frac{1}{15}I(\omega_{1},\omega_{3},t_{2})_{ijij}+
\frac{1}{15}I(\omega_{1},\omega_{3},t_{2})_{ijji}.
\end{split}
\end{equation}
This is the isotropically averaged parallel polarized spectrum.
Note that in Hochstrasser's paper, he is averaging over the molecular reference frame, while we are averaging over the laboratory frame.
The result should be independent of the reference frame.
Likewise, we can find the isotropically averaged signals
\begin{equation}
\begin{split}
\left\langle I(\omega_{1},\omega_{3},t_{2})_{ZZXX}\right\rangle=
\sum_{i,j={X,Y,Z}}
&\frac{1}{15}I(\omega_{1},\omega_{3},t_{2})_{iiii}+
\frac{2}{15}I(\omega_{1},\omega_{3},t_{2})_{iijj}+\\
&-\frac{1}{30}I(\omega_{1},\omega_{3},t_{2})_{ijij}+
-\frac{1}{30}I(\omega_{1},\omega_{3},t_{2})_{ijji}.
\end{split}
\end{equation}
This is the isotropically averaged perpendicularly polarized spectrum.
\begin{equation}
\begin{split}
-\frac{1}{2}\left\langle I(\omega_{1},\omega_{3},t_{2})_{ZXXZ} - I(\omega_{1},\omega_{3},t_{2})_{ZXZX}\right\rangle=&\\
\sum_{i,j={X,Y,Z}}
\frac{5}{60}I(\omega_{1},\omega_{3},t_{2})_{ijij}
&-\frac{5}{60}I(\omega_{1},\omega_{3},t_{2})_{ijji}.
\end{split}
\end{equation}
This is the isotropically averaged cross polarized spectrum.
